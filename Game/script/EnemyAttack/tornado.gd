...
func start(_AttackRange, _DamageCD, _Damage, _PullForce):
...
func _process(delta):
	...
	if attack_started and not setup:
		if timer_damage == 0:
			for body in player:
				body.receive_special_damage(Damage)
				body.velocity += body.position.direction_to(global_position) * PullForce
				body.move()
			timer_damage = DamageCD
